...
int n, m, s, t, a, b, ds[1010], dt[1010];
vpii e[1010];
...
signed main() {
	...
	rep(i,m) {
		cin >> a >> b;
		a--, b--;
		e[a].pb({b,1}), e[b].pb({a,1});
		if (a>b) swap(a,b);
		r.insert({a,b});
	}
	dijkstra(n,e,ds,s), dijkstra(n,e,dt,t);
	rep(i,n) reps(j,i+1,n) {
		if (r.count({i,j})) continue;
		if (ds[i]+1+dt[j]>=ds[t] && ds[j]+1+dt[i]>=ds[t]) res++;
	}
	cout << res << ln;
}