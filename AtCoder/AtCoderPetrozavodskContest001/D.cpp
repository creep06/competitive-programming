...
signed main() {
	...
	// {2番目に低いコスト,コストの最小値}とcs番号をキーにしてプライオリティーキューに突っ込む
	...
	int ans = 0;
	while (que.size()>1) {
		pair<pii,int> sm = que.top(); que.pop();
		pair<pii,int> bg = que.top(); que.pop();
